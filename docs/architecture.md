...
- Поле ACF `event_location_text` заполняется полным значением из столбца `LOCATION (CITY)` без обрезки по запятой; если он пустой — значение не передаётся в WordPress.
- Для PT-версии продукта поле ACF `event_location_text` также обновляется из `LOCATION (CITY)`, чтобы город был синхронизирован с EN-версией.
...
- При объединении `last_variations` в общий `attr_payload` имена атрибутов и значения опций дополнительно нормализуются (`strip()`), чтобы исключить дубли ключей вида `Running` и `Running `.
