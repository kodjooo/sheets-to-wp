...
- Категории/подкатегории продукта собираются из ВСЕХ строк блока гонки (главная строка + строки-вариаций), а не только из главной: подкатегории `SUBCATEGORY` каждой строки добавляются в `extra_categories`. Иначе применяется лишь подкатегория главной строки (напр. только Trail Run вместо Trail Run + Walking + Kids).
