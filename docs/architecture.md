...
- Поле ACF `event_location_text` заполняется значением из столбца `LOCATION (CITY)`; если он пустой — берётся первая часть из `LOCATION`.
...
## Развертывание
- Запуск через `docker-compose.yml`; код монтируется в `/app/run`, логи — в `/app/logs`.
