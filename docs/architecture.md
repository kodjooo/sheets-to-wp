...
- Подкатегории продукта собираются из всех строк блока гонки (главная + строки-вариаций): `SUBCATEGORY` каждой строки идёт в `extra_categories`. Со строк-вариаций берём подкатегорию ТОЛЬКО если её `CATEGORY` совпадает с категорией главной строки — так добираются подкатегории того же типа (Trail Run + Walking + Kids под Running), но чужая категория со строки-вариации (напр. Cycling у беговой гонки) не засоряет продукт. Именно межкатегорийное засорение убирал коммит f1ba788; восстановлен только сбор в пределах той же категории.
